...
app.post('/webhook/retell', async (req, res) => {
  try {
    ...
    console.log('Event:', event.event, '| Call:', event.data?.call_id);
    if (event.event !== 'call_analyzed') return res.status(200).json({ status: 'ignored' });
    const callData = event.data;
    const custom = callData.call_analysis?.custom_analysis_data || {};
    if (custom.commande_confirmee !== 'oui') return res.status(200).json({ status: 'ignored', reason: 'non_confirmee' });
    const numeroCommande = generateOrderNumber();
    const type = custom.type_commande || 'emporter';
    const fields = {
      ...
      'Montant (€)': parseFloat(custom.montant_total) || 0,
...
app.get('/ticket/:orderId', async (req, res) => {
  try {
    ...
    res.send(`<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><title>Ticket ${f['N° Commande']}</title>
...
<div class="center"><div style="font-size:24px">🍕</div><div class="bold large">PIZZA TRATTORIA</div><div class="sep"></div><div class="badge">${f['N° Commande']}</div><div style="font-size:11px">${now}</div></div>
<div class="sep"></div>
...
<div class="sep"></div>
<div class="bold">COMMANDE :</div><div class="detail">${f['Détail Commande']||'N/A'}</div>
<div class="sep"></div>
<div class="row"><span class="total">TOTAL :</span><span class="total">${f['Montant (€)']?parseFloat(f['Montant (€)']).toFixed(2)+' €':'N/A'}</span></div>
<div class="sep"></div>
<div class="center"><div class="bold">⏱ Délai : ${f['Délai']||'N/A'}</div><div style="font-size:10px;margin-top:8px">Merci ! Pizza Trattoria 🍕</div></div>
<div class="sep no-print"></div><div class="center no-print" style="margin:15px 0"><button onclick="window.print()" style="background:#dc2626;color:#fff;border:none;padding:10px 24px;font-size:15px;border-radius:6px;cursor:pointer;font-weight:bold">🖨️ IMPRIMER</button></div>
</body></html>`);
...
app.get('/dashboard', async (req, res) => {
  try {
    ...
    res.send(`<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>🍕 Pizza Trattoria POS</title>
...
<script>let cd=30;setInterval(()=>{cd--;const el=document.getElementById('cd');if(el)el.textContent=cd+'s';if(cd<=0)location.reload();},1000);async function updateStatus(id,status){try{await fetch('/update-status',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({recordId:id,status})});}catch(e){}}</script>
</head><body>
<div class="header"><span style="font-size:30px">🍕</span><div><h1>Pizza Trattoria — POS Dashboard</h1><div style="font-size:12px;opacity:.8">Refresh auto dans <span id="cd">30</span>s</div></div><div style="margin-left:auto"><button class="btn" onclick="location.reload()">↻ Refresh</button></div></div>
<div class="container">
<div class="stats">
<div class="card red"><div class="val">${todayRecs.length}</div><div class="lbl">Commandes aujourd'hui</div></div>
<div class="card blue"><div class="val">${todayRecs.filter(r=>r.fields.Type==='Livraison').length}</div><div class="lbl">Livraisons</div></div>
<div class="card green"><div class="val">${todayRecs.filter(r=>r.fields.Type==='Emporter').length}</div><div class="lbl">À emporter</div></div>
<div class="card orange"><div class="val">${totalJour.toFixed(2)} €</div><div class="lbl">CA du jour</div></div>
</div>
<div class="table-wrap">
<div style="padding:14px 20px;border-bottom:1px solid #f3f4f6;display:flex;justify-content:space-between;align-items:center"><strong>📋 Commandes (${records.length})</strong><span style="font-size:12px;color:#9ca3af">Triées par date</span></div>
<table><thead><tr><th>N° Commande</th><th>Client</th><th>Téléphone</th><th>Type</th><th>Détail</th><th>Montant</th><th>Statut</th><th>Date</th><th>Ticket</th></tr></thead>
...
app.get('/', (req, res) => res.json({ status: 'ok', service: 'Pizza Trattoria POS', version: '1.0.0' }));
