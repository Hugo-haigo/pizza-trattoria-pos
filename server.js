...
app.post('/webhook/retell', async (req, res) => {
  try {
    ...
    const fields = {
      ...
      'Type': type.charAt(0).toUpperCase() + type.slice(1), 'Adresse Livraison': custom.adresse_livraison || '',
      ...
      'D\u00e9lai': type === 'livraison' ? '50 min' : '35 min', 'Statut': 'Nouvelle',
      'Call ID': callId, 'Dur\u00e9e Appel': callData.duration_ms ? Math.round(callData.duration_ms/1000)+'s' : 'N/A',
    ...
    console.log('OK:', numeroCommande, r.data.id);
...
app.get('/ticket/:orderId', async (req, res) => {
  try {
    ...
    res.send('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Ticket</title><style>body{font-family:monospace;max-width:310px;margin:20px auto;padding:15px;font-size:13px}.c{text-align:center}.b{font-weight:bold}.s{border-top:2px dashed #000;margin:10px 0}.r{display:flex;justify-content:space-between;margin:3px 0}.bg{background:#000;color:#fff;padding:5px 12px;border-radius:4px;display:inline-block}.t{font-size:20px;font-weight:bold}@media print{.np{display:none!important}body{margin:0;max-width:100%}}</style></head><body><div class="c"><div style="font-size:24px">&#127829;</div><div class="b" style="font-size:18px">PIZZA TRATTORIA</div><div class="s"></div><div class="bg">' + nCmd + '</div><div style="font-size:11px">' + new Date().toLocaleString('fr-FR') + '</div></div><div class="s"></div><div class="r"><span class="b">Client:</span><span>' + (f['Nom Client']||'N/A') + '</span></div><div class="r"><span class="b">Tel:</span><span>' + (f['T\u00e9l\u00e9phone']||'N/A') + '</span></div><div class="r"><span class="b">Mode:</span><span>' + (f['Type']||'N/A') + '</span></div>' + (f['Adresse Livraison']?'<div class="r"><span class="b">Adresse:</span><span>'+f['Adresse Livraison']+'</span></div>':'') + '<div class="s"></div><div class="b">COMMANDE:</div><div style="background:#f5f5f5;padding:8px;white-space:pre-wrap;font-size:12px">' + (f['D\u00e9tail Commande']||'N/A') + '</div><div class="s"></div><div class="r"><span class="t">TOTAL:</span><span class="t">' + montant + '</span></div><div class="s"></div><div class="c"><div class="b">Delai: ' + (f['D\u00e9lai']||'N/A') + '</div></div><div class="s np"></div><div class="c np" style="margin:15px 0"><button onclick="window.print()" style="background:#dc2626;color:#fff;border:none;padding:10px 24px;font-size:15px;border-radius:6px;cursor:pointer">IMPRIMER</button></div></body></html>');
...
app.get('/dashboard', (req, res) => {
  ...
  res.end('<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Pizza Trattoria POS</title>' +
  '<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;background:#f9fafb;color:#111827}' +
  ...
  '</style></head><body>' +
  '<div class="hdr"><span style="font-size:26px">&#127829;</span><div><h1>Pizza Trattoria &#8212; POS</h1>' +
  ...
  '</div><div class="panel"><div class="phead"><strong>Commandes (<span id="cnt">...</span>)</strong><span style="font-size:11px;color:#9ca3af">Par date</span></div>' +
  '<div class="scroll"><table><thead><tr><th>N&#176;</th><th>Client</th><th>T&#233;l</th><th>Type</th><th>D&#233;tail</th><th>Montant</th><th>Statut</th><th>Date</th><th>Ticket</th></tr></thead>' +
  '<tbody id="tb"><tr class="empty"><td colspan="9">Chargement...</td></tr></tbody></table></div></div></div>' +
  '<script>\n' +
  ...
  'function startCD(){clearInterval(cdi);cd=30;setText("cd","30s");cdi=setInterval(function(){cd--;setText("cd",cd+"s");if(cd<=0){clearInterval(cdi);load();}},1000);}\n' +
  ...
  '  var tv=ta.filter(function(r){var s=r.fields.Statut;return s!=="Annul\u00e9e"&&s!=="Annulee";});\n' +
  ...
  '  var tb=_("tb");\n' +
  '  if(!rec||!rec.length){tb.innerHTML="<tr class=\"empty\"><td colspan=\"9\">Aucune commande</td></tr>";return;}\n' +
  '  var html="";\n' +
  '  for(var i=0;i<rec.length;i++){\n' +
  '    var r=rec[i],f=r.fields,rid=r.id;\n' +
  '    var st=f.Statut||"Nouvelle";\n' +
  ...
  '    var nc=f["N\u00b0 Commande"]||"-";\n' +
  '    var isAnn=(st==="Annul\u00e9e"||st==="Annulee");\n' +
  '    var statList=["Nouvelle","En pr\u00e9paration","Pr\u00eate","Livr\u00e9e","Annul\u00e9e"];\n' +
  '    var opts="";for(var j=0;j<statList.length;j++){var sx=statList[j];opts+=\'<option value="\'+sx+\'"\'+(sx===st?\" selected\":\"\")+\'>\'+sx+\'</option>\'; }\n' +
  '    html+=\'<tr class="\'+( isAnn?"ann":"")+\'">\';\n' +
  '    html+=\'<td style="font-weight:bold;font-family:monospace">\'+nc+\'</td>\';\n' +
  '    html+=\'<td>\'+( f["Nom Client"]||"-")+\'</td>\';\n' +
  '    html+=\'<td>\'+( f["T\u00e9l\u00e9phone"]||"-")+\'</td>\';\n' +
  '    html+=\'<td><span class="badge" style="background:\'+tc+\'">\'+( f.Type||"-")+\'</span></td>\';\n' +
  '    html+=\'<td style="font-size:11px;max-width:200px">\'+det+\'</td>\';\n' +
  '    html+=\'<td style="font-weight:bold;color:#dc2626">\'+mo+\'</td>\';\n' +
  '    html+=\'<td><select style="border-color:\'+cl+\';color:\'+cl+\'"\'+ \'onchange="updateSt(\\\'\'+rid+\'\\\'"+",this)">\'+opts+\'</select></td>\';\n' +
  '    html+=\'<td style="font-size:11px;white-space:nowrap">\'+dt+\'</td>\';\n' +
  '    html+=\'<td><a class="tlink" href="/ticket/\'+encodeURIComponent(nc)+\'" target="_blank">Ticket</a></td>\';\n' +
  '    html+=\'</tr>\';\n' +
  '  }\n' +
  '  tb.innerHTML=html;\n' +
  '}\n' +
  ...
  '  if(row)row.className=(s==="Annul\u00e9e"||s==="Annulee")?"ann":"";\n' +
...
app.get('/', (req, res) => res.json({ status: 'ok', service: 'Pizza Trattoria POS', version: '2.4.0' }));
